...
impl NetworkSerial {
    pub fn to_string(&self) -> String {
        let proto = match self.protocol {
            ...
            TransportType::Usb => "usb", // 不应该出现，但还是处理一下
        };
...
fn parse_host_port(s: &str, protocol: TransportType) -> Result<(String, u16), FastbootError> {
    if s.starts_with('[') {
        let bracket_end = s.find(']').ok_or_else(|| {
            FastbootError::InvalidArg(format!("IPv6 地址格式错误，缺少 ]: {}", s))
        })?;
...
fn parse_port(s: &str) -> Result<u16, FastbootError> {
    s.parse().map_err(|_| {
        FastbootError::InvalidArg(format!("无效的端口号: {}", s))
    })
}
...
pub fn parse_hex_u64(s: &str) -> Result<u64, FastbootError> {
    ...
    u64::from_str_radix(s, 16).map_err(|_| {
        FastbootError::InvalidArg(format!("无效的十六进制数: {}", s))
    })
}
...
#[test]
    ...
    fn test_is_network_serial() {
        ...
        assert!(!is_network_serial("ABC123")); // USB 序列号
