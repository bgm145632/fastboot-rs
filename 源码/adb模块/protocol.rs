...
#[repr(u32)]
pub enum AdbCommand {
    Cnxn = 0x4e584e43,  // "CNXN" - 连接
    Auth = 0x48545541,  // "AUTH" - 认证
    Open = 0x4e45504f,  // "OPEN" - 打开流
    Okay = 0x59414b4f,  // "OKAY" - 确认/就绪
    Clse = 0x45534c43,  // "CLSE" - 关闭流
    Wrte = 0x45545257,  // "WRTE" - 写数据
}
...
impl AdbMessage {
    ...
    pub fn encode_header(&self) -> Vec<u8> {
        ...
        buf.write_u32::<LittleEndian>(Self::checksum(&self.data)).unwrap();
    ...
    pub fn decode_header(data: &[u8]) -> io::Result<(AdbCommand, u32, u32, u32, u32)> {
        ...
        let command = AdbCommand::from_u32(cmd)
            .ok_or_else(|| io::Error::new(io::ErrorKind::InvalidData, format!("未知命令: 0x{:08x}", cmd)))?;
    ...
    pub fn decode(header: &[u8], payload: &[u8]) -> io::Result<Self> {
        ...
        if payload.len() != data_len as usize {
            return Err(io::Error::new(
                ...
                format!("payload 长度不匹配: 期望 {}, 实际 {}", data_len, payload.len()),
            ));
        }
