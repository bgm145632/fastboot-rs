...
impl<T: AdbTransport> AdbConnection<T> {
            ...
            match response.command {
                ...
                AdbCommand::Auth => {
                    ...
                    if self.key_manager.is_none() {
                        self.key_manager = Some(AdbKeyManager::load_or_generate()?);
                    }
                    let key_manager = self.key_manager.as_ref().unwrap();

                    if auth_type == 1 {
                        if !signature_sent {
                            let signature = key_manager.sign(&response.data)?;
                            let auth_msg = AdbMessage::auth(AuthType::Signature, signature);
                            self.send_message(&auth_msg)?;
                            signature_sent = true;
                        } else if self.wait_for_auth && !pubkey_sent {
                            eprintln!("等待设备授权... 请在设备上点击「允许」");
                            let pubkey = key_manager.get_adb_public_key()?;
                            let auth_msg = AdbMessage::auth(AuthType::RsaPublic, pubkey);
                            self.send_message(&auth_msg)?;
                            pubkey_sent = true;
                        } else if self.wait_for_auth && pubkey_sent {
                            let signature = key_manager.sign(&response.data)?;
    ...
    fn handle_auth(&mut self, msg: &AdbMessage) -> io::Result<bool> {
        ...
        match auth_type {
            1 => {
                let signature = key_manager.sign(&msg.data)?;
                let auth_msg = AdbMessage::auth(AuthType::Signature, signature);
                self.send_message(&auth_msg)?;
                Ok(false)
            }
    ...
    pub fn write_stream(&mut self, local_id: u32, data: &[u8]) -> io::Result<()> {
        ...
        if !stream.is_open {
            return Err(io::Error::new(io::ErrorKind::NotConnected, "流已关闭"));
        }
                ...
                match response.command {
                    ...
                    AdbCommand::Clse => {
                        if response.arg1 == local_id {
                            ...
                            return Err(io::Error::new(io::ErrorKind::BrokenPipe, "流被关闭"));
                        }
                    }
                    _ => {}
                }
    ...
    fn send_message(&mut self, msg: &AdbMessage) -> io::Result<()> {
        ...
        if !msg.data.is_empty() {
            self.transport.write_all(&msg.data)
                .map_err(|e| transport_error_to_io_error(e))?;
        }
    ...
    pub fn try_read_stream(&mut self, local_id: u32, timeout_ms: u64) -> io::Result<Option<Vec<u8>>> {
        let stream = self.streams.get(&local_id)
            .ok_or_else(|| io::Error::new(io::ErrorKind::NotFound, "流不存在"))?;
...
fn transport_error_to_io_error(e: TransportError) -> io::Error {
    match e {
        TransportError::Timeout => io::Error::new(io::ErrorKind::TimedOut, "操作超时"),
        TransportError::Disconnected => io::Error::new(io::ErrorKind::NotConnected, "设备断开"),
        TransportError::NoLink => io::Error::new(io::ErrorKind::NotConnected, "连接丢失"),
