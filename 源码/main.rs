...
#[path = "协议层/driver.rs"]
mod driver;
#[path = "协议层/protocol.rs"]
mod protocol;
#[path = "协议层/sparse.rs"]
mod sparse;

#[path = "传输层/transport.rs"]
mod transport;
#[path = "传输层/tcp.rs"]
mod tcp_transport;
#[path = "传输层/udp.rs"]
...
#[path = "功能层/flash.rs"]
mod flash;
#[path = "功能层/partition.rs"]
mod partition;
#[path = "功能层/progress.rs"]
mod progress;
#[path = "功能层/util.rs"]
mod util;
...
async fn cmd_adb_auth(serial: &Option<String>) -> Result<(), FastbootError> {
    use adb::client::AdbClient;

    println!("正在申请 ADB 授权...");
    println!("请在设备上确认授权对话框");
    println!();

    let client = AdbClient::connect_with_auth(serial.as_deref())
        .map_err(|e| FastbootError::Adb(e.to_string()))?;

    let mut client = client;
    let serial = client.serial().to_string();

    let model = client.get_prop("ro.product.model")
        .unwrap_or_else(|_| "Unknown".to_string());

    print_success(&format!("授权成功!"));
    println!("  设备: {}", serial);
    println!("  型号: {}", model);
