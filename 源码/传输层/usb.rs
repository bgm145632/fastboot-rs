...
mod windows_impl {
    ...
    const ADB_DEVICE_GUID: GUID = GUID {
        Data1: 0xF72FE0D4, Data2: 0xCBCB, Data3: 0x407D,
    ...
    const ADB_IOCTL_GET_USB_DEVICE_DESCRIPTOR: DWORD =
        ctl_code(FILE_DEVICE_UNKNOWN, ADB_CTL_GET_USB_DEVICE_DESCRIPTOR, METHOD_BUFFERED, FILE_READ_ACCESS);
    const ADB_IOCTL_GET_SERIAL_NUMBER: DWORD =
        ctl_code(FILE_DEVICE_UNKNOWN, ADB_CTL_GET_SERIAL_NUMBER, METHOD_BUFFERED, FILE_READ_ACCESS);
    #[allow(dead_code)]
    const ADB_IOCTL_BULK_READ: DWORD =
        ctl_code(FILE_DEVICE_UNKNOWN, ADB_CTL_BULK_READ, METHOD_OUT_DIRECT, FILE_READ_ACCESS);
    #[allow(dead_code)]
    const ADB_IOCTL_BULK_WRITE: DWORD =
        ctl_code(FILE_DEVICE_UNKNOWN, ADB_CTL_BULK_WRITE, METHOD_BUFFERED, FILE_WRITE_ACCESS);

    use winapi::shared::winusbio::WINUSB_PIPE_INFORMATION;
    use winapi::um::winusb::{
        WinUsb_Initialize, WinUsb_Free, WinUsb_QueryPipe,
        WinUsb_SetPipePolicy, WinUsb_ReadPipe, WinUsb_WritePipe,
        WinUsb_GetOverlappedResult, WinUsb_FlushPipe, WinUsb_ResetPipe,
        WinUsb_QueryInterfaceSettings, WinUsb_AbortPipe,
        WINUSB_INTERFACE_HANDLE, USB_INTERFACE_DESCRIPTOR,
    };
...
#[repr(C, packed)]
    ...
    impl UsbTransport {
        ...
        fn enumerate_devices_by_mode(mode_filter: Option<UsbMode>) -> Result<Vec<UsbDeviceInfo>, TransportError> {
            ...
            unsafe {
                ...
                while SetupDiEnumDeviceInterfaces(
                    ...
                    if let Some(info) = Self::get_device_info(dev_info, &mut dev_interface_data) {
                        ...
                        match mode_filter {
                            Some(UsbMode::Fastboot) if actual_mode == Some(UsbMode::Fastboot) => devices.push(info),
                            Some(UsbMode::Adb) if actual_mode == Some(UsbMode::Adb) => devices.push(info),
        ...
        unsafe fn get_device_info(
            ...
            let device_path_len = (0..).take_while(|&i| *device_path_ptr.offset(i) != 0).count();
            let device_path_slice = std::slice::from_raw_parts(device_path_ptr, device_path_len);
            let device_path = OsString::from_wide(device_path_slice).to_string_lossy().to_string();
        ...
        fn get_serial_via_ioctl(device_path: &str) -> Option<String> {
            unsafe {
                ...
                if result != 0 && bytes_returned > 0 {
                    let len = serial_buf.iter().position(|&c| c == 0).unwrap_or(serial_buf.len());
...
let vid_pos = path_lower.find("vid_")?;
            ...
            let pid_end = path_lower[pid_pos + 4..].find('&').map(|p| pid_pos + 4 + p).unwrap_or(pid_pos + 8);
        ...
        fn open_device_with_mode(info: &UsbDeviceInfo, mode: UsbMode) -> Result<Self, TransportError> {
            unsafe {
                ...
                match driver_type {
                    ...
                    DriverType::Unknown => {
                        Self::open_winusb_device(info, &path, mode)
                            .or_else(|_| Self::open_legacy_device(info, &path))
                            .map_err(|_| TransportError::Usb(
                                "无法识别设备驱动类型，WinUSB 和 Legacy 模式都失败。\n\
                                 建议：安装 Google USB Driver。\n\
                                 下载地址: https://developer.android.com/studio/run/win-usb".into()
                            ))
                    }
                }
            }
        ...
        fn open_legacy_device(info: &UsbDeviceInfo, path: &[u16]) -> Result<Self, TransportError> {
            unsafe {
                ...
                Ok(Self {
                    ...
                    max_packet_size: if info.is_usb3 { USB3_MAX_PACKET_SIZE } else { USB2_MAX_PACKET_SIZE },
        ...
        fn open_winusb_device(info: &UsbDeviceInfo, path: &[u16], mode: UsbMode) -> Result<Self, TransportError> {
...
for i in 0..iface_desc.bNumEndpoints {
        ...
        fn winusb_write(&mut self, data: &[u8]) -> Result<usize, TransportError> {
            unsafe {
                let endpoints = self.winusb_endpoints.ok_or_else(|| {
                    TransportError::Usb("WinUSB 端点未初始化".into())
                })?;
        ...
        fn winusb_read(&mut self, buf: &mut [u8]) -> Result<usize, TransportError> {
            unsafe {
                ...
                let timeout_ms = self.timeout.as_millis() as DWORD;
                let actual_timeout = if timeout_ms == 0 { INFINITE } else { timeout_ms };
                ...
                if get_result == 0 {
                    ...
                    return Err(TransportError::Usb(format!(
                        "WinUSB 读取等待失败: {} (错误码: {})",
                        std::io::Error::last_os_error(), err
                    )));
                }
    ...
    impl AsyncTransport for UsbTransport {
        ...
        fn reinitialize(&mut self) -> Pin<Box<dyn Future<Output = Result<(), TransportError>> + Send + '_>> {
            Box::pin(async move {
                if self.driver_type != DriverType::WinUsb {
                    return Ok(());
                }
...
mod unix_impl {
    ...
    impl UsbTransport {
                    ...
                    for iface_desc in iface.descriptors() {
                            ...
                            && iface_desc.protocol_code() == FASTBOOT_PROTOCOL
                        {
                            ...
                            return Some(UsbDeviceInfo {
                                ...
                                product_id: pid,
                                serial_number: serial,
                                device_path: format!("{}:{}", device.bus_number(), device.address()),
                                is_usb3,
                                product_name,
                                manufacturer,
                            });
        ...
        pub fn open_adb(serial: Option<&str>) -> Result<Self, TransportError> {
            ...
            let device_info = match serial {
                Some(s) => devices
                    .into_iter()
                    .find(|d| d.serial_number == s)
                    .ok_or_else(|| TransportError::Usb(format!("ADB device '{}' not found", s)))?,
                None => {
                    if devices.is_empty() {
                        return Err(TransportError::Usb("No ADB devices found".into()));
                    }
                    if devices.len() > 1 {
                        return Err(TransportError::Usb(
                            "Multiple ADB devices found, use -s to specify".into(),
                        ));
                    }
                    devices.into_iter().next().unwrap()
                }
        ...
        fn open_adb_device(info: &UsbDeviceInfo) -> Result<Self, TransportError> {
            ...
            if handle.kernel_driver_active(iface_num).unwrap_or(false) {
                handle
                    ...
                    .map_err(|e| TransportError::Usb(format!("Cannot detach kernel driver: {}", e)))?;
        ...
        fn find_adb_interface(device: &Device<Context>) -> Result<(u8, u8, u8), TransportError> {
            ...
            Err(TransportError::Usb("ADB interface not found".into()))
        }
        ...
        fn find_fastboot_interface(device: &Device<Context>) -> Result<(u8, u8, u8), TransportError> {
            let desc = device
                .device_descriptor()
                .map_err(|e| TransportError::Usb(e.to_string()))?;
