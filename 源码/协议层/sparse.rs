...
impl MappedSparseFile {
    ...
    pub fn iter_chunks(&self) -> impl Iterator<Item = (ChunkType, &[u8])> + '_ {
        self.chunks.iter().enumerate().filter_map(|(i, chunk)| {
            self.chunk_data(i).map(|data| (chunk.chunk_type, data))
        })
    }
}
...
impl<R: Read + Seek> StreamingSparseFile<R> {
    pub fn new(mut reader: R) -> Result<Self, SparseError> {
        ...
        reader.read_exact(&mut header_buf).map_err(SparseError::Io)?;
...
impl StreamingResparse {
    pub fn new(path: &std::path::Path, max_fragment_size: u64) -> Result<Self, SparseError> {
        ...
        for _ in 0..header.total_chunks {
            file.seek(SeekFrom::Start(offset)).map_err(SparseError::Io)?;
        ...
        let fragment_ranges = Self::calculate_fragment_ranges(
            &chunk_metas,
            &header,
            max_fragment_size,
        );
    ...
    fn calculate_fragment_ranges(
        ...
        while chunk_idx < chunk_metas.len() {
            let start_chunk = chunk_idx;
            let start_block = chunk_metas[chunk_idx].start_block;
            let mut current_size = 0u64;
            let mut end_block = start_block;
            let first_chunk_offset = 0;
            let mut last_chunk_size = 0;

            while chunk_idx < chunk_metas.len() {
                let meta = &chunk_metas[chunk_idx];
                let chunk_transfer_size = match meta.chunk_type {
                    ChunkType::Raw => CHUNK_HEADER_SIZE as u64 + meta.data_size as u64,
                    ChunkType::Fill => CHUNK_HEADER_SIZE as u64 + 4,
                    ChunkType::DontCare => CHUNK_HEADER_SIZE as u64,
                    ChunkType::Crc32 => CHUNK_HEADER_SIZE as u64 + 4,
                };

                if current_size + chunk_transfer_size > effective_max && chunk_idx > start_chunk {
                    break;
                }

                if meta.chunk_type == ChunkType::Raw && current_size + chunk_transfer_size > effective_max {
                    let remaining_space = (effective_max - current_size) as usize;
                    if remaining_space < CHUNK_HEADER_SIZE + block_size as usize {
                        break;
                    }

                    let max_data = remaining_space - CHUNK_HEADER_SIZE;
                    let aligned_data = (max_data / block_size as usize) * block_size as usize;

                    if aligned_data > 0 && aligned_data < meta.data_size as usize {
                        last_chunk_size = aligned_data;
                        end_block += (aligned_data / block_size as usize) as u32;
                        chunk_idx += 1;
                        break;
                    }
                }

                current_size += chunk_transfer_size;
                end_block += meta.output_blocks;
                last_chunk_size = meta.data_size as usize;
                chunk_idx += 1;
            }
    ...
    fn build_fragment(&self, file: &mut File, range: &FragmentRange) -> Result<Vec<u8>, SparseError> {
        ...
        if start_block > 0 {
            num_chunks += 1;  // 开头 DONT_CARE
        }
        if trailing_blocks > 0 {
            num_chunks += 1;  // 末尾 DONT_CARE
        }
        ...
        for i in range.start_chunk..range.end_chunk {
            ...
            estimated_size += match meta.chunk_type {
                ChunkType::Raw => {
                    if i == range.end_chunk - 1 && range.last_chunk_size < meta.data_size as usize {
        ...
        buffer.extend_from_slice(&1u16.to_le_bytes());  // major_version
        buffer.extend_from_slice(&0u16.to_le_bytes());  // minor_version
        ...
        buffer.extend_from_slice(&0u32.to_le_bytes());  // checksum
        ...
        for i in range.start_chunk..range.end_chunk {
            ...
            match meta.chunk_type {
                ChunkType::Raw => {
                    let data_size = if i == range.end_chunk - 1 && range.last_chunk_size < meta.data_size as usize {
                        range.last_chunk_size
                    } else {
                        meta.data_size as usize
                    };

                    let output_blocks = ((data_size + block_size as usize - 1) / block_size as usize) as u32;
                    ...
                    file.seek(SeekFrom::Start(meta.file_offset)).map_err(SparseError::Io)?;
                    let old_len = buffer.len();
                    buffer.resize(old_len + data_size, 0);
                    file.read_exact(&mut buffer[old_len..]).map_err(SparseError::Io)?;
                ...
                ChunkType::Crc32 => {
                    ...
                    buffer.extend_from_slice(&0u16.to_le_bytes());
                    buffer.extend_from_slice(&0u32.to_le_bytes());  // CRC chunk 输出 0 块
                    buffer.extend_from_slice(&total_sz.to_le_bytes());
...
impl SparseWriter {
    ...
    pub fn build(&self) -> Vec<u8> {
        ...
        output.extend_from_slice(&1u16.to_le_bytes()); // major_version
        output.extend_from_slice(&0u16.to_le_bytes()); // minor_version
        ...
        output.extend_from_slice(&0u32.to_le_bytes()); // checksum
...
pub fn resparse(sparse: &MappedSparseFile, max_size: u64) -> Result<Vec<SparseFragment>, SparseError> {
    ...
    for (i, chunk_info) in sparse.chunks().iter().enumerate() {
        let chunk_data = sparse.chunk_data(i).ok_or_else(|| {
            SparseError::InvalidHeader(format!("无法读取 chunk {} 的数据", i))
        })?;
        ...
        let chunk = match chunk_info.chunk_type {
            ...
            ChunkType::Fill => {
                ...
                SparseChunk::Fill { value, blocks: chunk_info.output_blocks }
            }
            ChunkType::DontCare => SparseChunk::DontCare { blocks: chunk_info.output_blocks },
            ChunkType::Crc32 => {
    ...
    while chunk_idx < all_chunks.len() {
        ...
        while chunk_idx < all_chunks.len() {
            ...
            if fragment_transfer_size + chunk_transfer > effective_max_size && !fragment_chunks.is_empty() {
                break;
            }
            ...
            if let SparseChunk::Raw(ref data) = chunk_with_offset.chunk {
                ...
                if remaining_space < CHUNK_HEADER_SIZE + block_size as usize && !fragment_chunks.is_empty() {
                    break;
                }
        ...
        data.extend_from_slice(&total_blks.to_le_bytes());  // 整个镜像的总块数
        data.extend_from_slice(&num_chunks.to_le_bytes());
        data.extend_from_slice(&0u32.to_le_bytes());  // checksum

        if fragment_start_block > 0 {
            data.extend_from_slice(&0xCAC3u16.to_le_bytes());  // DONT_CARE
...
pub fn validate_sparse_file(path: &Path) -> SparseValidation {
    ...
    if file_size < SPARSE_HEADER_SIZE as u64 {
        return SparseValidation {
            ...
            calculated_blocks: 0,
            calculated_transfer_size: 0,
            error: Some(format!("文件太小，无法包含 Sparse 头 (需要 {} 字节，实际 {} 字节)",
    ...
    for i in 0..header.total_chunks {
        if offset as usize + CHUNK_HEADER_SIZE > mmap.len() {
            return SparseValidation {
                ...
                error: Some(format!("Chunk {} 头超出文件范围 (偏移 {}, 文件大小 {})",
                    i, offset, file_size)),
            };
        }
        ...
        if chunk_header.get_type().is_err() {
            return SparseValidation {
                ...
                calculated_blocks,
                calculated_transfer_size,
                error: Some(format!("Chunk {} 类型无效: 0x{:04X}", i, chunk_header.chunk_type)),
            };
        }
        ...
        if chunk_end > file_size {
            return SparseValidation {
                ...
                calculated_blocks,
                calculated_transfer_size,
                error: Some(format!("Chunk {} 数据超出文件范围 (结束位置 {}, 文件大小 {})",
    ...
    if calculated_blocks != header.total_blks {
        return SparseValidation {
            ...
            calculated_blocks,
            calculated_transfer_size,
            error: Some(format!("总块数不匹配: 头部声明 {} 块，实际计算 {} 块",
...
pub fn quick_validate_sparse(path: &Path) -> Result<SparseHeader, SparseError> {
    ...
    let min_size = header.file_hdr_sz as u64 +
        (header.total_chunks as u64 * CHUNK_HEADER_SIZE as u64);
...
mod tests {
    ...
    fn test_sparse_header_validate() {
        ...
        assert!(matches!(header.validate(), Err(SparseError::InvalidMagic(_))));
        ...
        assert!(matches!(header.validate(), Err(SparseError::UnsupportedVersion(_, _))));
        ...
        assert!(matches!(header.validate(), Err(SparseError::InvalidHeader(_))));
    }
    ...
    fn test_sparse_writer_basic() {
        ...
        writer.add_chunk(SparseChunk::Fill { value: 0, blocks: 5 });
        ...
        assert_eq!(header.total_blks, 10); // 1 + 5 + 4
    }
    ...
    fn test_sparse_writer_roundtrip() {
        ...
        writer.add_chunk(SparseChunk::Raw(vec![0x11; 4096 * 2])); // 2 blocks
        writer.add_chunk(SparseChunk::Fill { value: 0xDEADBEEF, blocks: 10 });
    ...
    fn test_sparse_chunk_transfer_size() {
        ...
        let fill = SparseChunk::Fill { value: 0, blocks: 100 };
    ...
    fn test_sparse_chunk_output_blocks() {
        ...
        let fill = SparseChunk::Fill { value: 0, blocks: 50 };
    ...
    fn test_resparse_no_split_needed() {
        ...
        let chunks = vec![
            ...
            SparseChunk::Fill { value: 0, blocks: 2 },
        ];
        let data = create_test_sparse(block_size, &chunks);

        let header = SparseHeader::from_bytes(&data).unwrap();
        assert!(header.validate().is_ok());
        assert_eq!(header.total_blks, 3); // 1 + 2
    }
    ...
    fn test_sparse_writer_preserves_expanded_size() {
        ...
        let test_cases = vec![
            ...
            vec![SparseChunk::Fill { value: 0xDEAD, blocks: 10 }],
            vec![SparseChunk::DontCare { blocks: 20 }],
            vec![
                SparseChunk::Raw(vec![0x11; 4096 * 2]),
                SparseChunk::Fill { value: 0, blocks: 5 },
    ...
    fn test_sparse_transfer_size_calculation() {
        let block_size = 4096u32;
        let chunks = vec![
            SparseChunk::Raw(vec![0xAB; 4096 * 2]), // 头 12 + 数据 8192
            SparseChunk::Fill { value: 0, blocks: 5 }, // 头 12 + 数据 4
            SparseChunk::DontCare { blocks: 3 }, // 头 12
        ];

        let expected_transfer = SPARSE_HEADER_SIZE as u64 // 28
            + (CHUNK_HEADER_SIZE as u64 + 4096 * 2) // Raw
            + (CHUNK_HEADER_SIZE as u64 + 4) // Fill
            + CHUNK_HEADER_SIZE as u64; // DontCare
...
mod property_tests {
    ...
    fn arb_sparse_chunk(block_size: u32) -> impl Strategy<Value = SparseChunk> {
        prop_oneof![
            ...
            (any::<u32>(), 1u32..=20).prop_map(|(value, blocks)| {
                SparseChunk::Fill { value, blocks }
            }),
            (1u32..=50).prop_map(|blocks| {
                SparseChunk::DontCare { blocks }
            }),
        ]
    }
